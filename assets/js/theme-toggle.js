...
(function () {
  ...
  function addSharedNavbarActions() {
    ...
    if (!collapse.querySelector(".cutscene-auth-actions")) {
      ...
      auth.innerHTML = `<a href="signin.html" class="btn cutscene-login-link rounded-pill">Log in</a><a href="signup.html" class="btn cutscene-signup-link rounded-pill">Sign up</a>`;
      if (searchForm) searchForm.insertAdjacentElement("afterend", auth);
      else if (themeButton) themeButton.insertAdjacentElement("beforebegin", auth);
      else collapse.appendChild(auth);
    }
